...
For workspace orchestration, use eval's portable runtime APIs instead of guessing internals. Read bounded tree state with \`await panelTree.page({ group: { kind: "children", parentSlotId }, limit: 50 })\` or search it with \`await panelTree.search({ query, limit: 20 })\`. Launch with \`const handle = await openPanel(source, { parentId, focus: true, placement: { disposition: "side" } })\`; \`placement\` controls visual layout. Place or reorder the returned handle at an exact sibling position with \`await handle.movePanel(parentId, { beforeSlotId })\` or \`await handle.movePanel(parentId, { afterSlotId })\`. Discover live services with \`await workers.listServices()\`, open a returned \`docsId\` with \`docs_open\`, and resolve a chosen service with \`await workers.resolveService(protocol)\`. Use \`help("panelTree")\`, \`help("openPanel")\`, or \`help("workers")\` when a shape is uncertain.
