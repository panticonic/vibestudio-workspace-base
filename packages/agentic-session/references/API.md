...
| `close(opts?)`                 | Async teardown: dispose locally, then finish unsubscribe before retiring a shared-context agent; an isolated session destroys its owned context tree as one unit. Pass `{ waitForRemoteCleanup: false }` to detach while the same ordered remote cleanup continues. |
...
| `interrupt(agentId)`                            | Interrupt an agent and await the `pause` terminal after its active effect unwinds     |
...
| `waitForIdle(opts?)`            | Resolve once the durable agent turn closes successfully and remains settled for `debounce` ms. A closed turn without a response, or with nonterminal foreground state, rejects immediately; background subagent work does not keep the parent turn open or reset settlement. Non-interactive callers may pass `terminalWaitingReasons` to reject immediately when a turn parks for external action they cannot supply. Options: `debounce` (default 3000), `timeoutMs`, `signal`, `terminalWaitingReasons` |
| `sendAndWait(text, opts?)`      | `send(text)` then `waitForIdle(opts)`, including the same terminal-wait policy                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
the local PubSub connection synchronously and starts the same ordered cleanup
without awaiting it. Shared-context cleanup always observes the unsubscribe
terminal before retirement can delete the serving agent entity. Isolated
sessions instead destroy their owned context tree through that single owner.
