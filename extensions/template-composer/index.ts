...
import {
  createAffectedBuildGate,
  TemplateAuthoringBuildError,
} from "./build.js";
...
function stagedOperationRecord(
...
export async function integrateTemplatePublicationIntoCallerContext(
  ...
  if (!contextId || contextId.startsWith(OPERATION_CONTEXT_PREFIX))
    return undefined;
  try {
    ...
    if (
      status.mainRelation === "at" &&
      status.mainEventId === publicationEventId
    ) {
      return { state: "integrated", contextId };
    }
    const source = { kind: "event" as const, eventId: publicationEventId };
    const compared = await ctx.rpc.call<VcsCompareResult>(
      "main",
      "vcs.compare",
      {
    ...
    if (compared.resolution.concluded)
      return { state: "integrated", contextId };
    if (!compared.resolution.complete)
      return { state: "needs-merge", contextId };
    ...
    return {
      state:
        merged.resolution.complete && merged.resolution.concluded
          ? "integrated"
          : "needs-merge",
      contextId,
    };
    ...
    ctx.log.warn?.(
      "Template publication could not be integrated into its caller context",
      {
        operationId,
        contextId,
...
async function operationRecordForMutation(
  ...
  if (
    await isTemplateOperationCancelled(
      ctx,
    ...
    throw new Error(
      `Template operation ${operationId} was cancelled and cannot be reused`,
    );
  }
...
async function completedOperationResult(
  ...
  const contextIntegration =
    await integrateTemplatePublicationIntoCallerContext(
      ctx,
      record.operationId,
      status.mainEventId,
    );
...
async function resolveAddSource(
  ...
  if ("catalogId" in request) {
    ...
    const registryCommit =
      request.registryCommit ?? env.catalog!.coordinates.commit;
  ...
  return {
    ...
    excludedSuggestions: inspection.plan.nodes.flatMap((node) =>
      (["trust", "providers"] as const).flatMap((section) => {
        ...
        if (
          suggestionDecisions?.[`${node.nodeId}:${section}`]?.digest === digest
        ) {
          return [];
        }
...
function affectedTemplateParts(
  ...
  const nextUrls = new Map(
    inspection.plan.nodes.map((node) => [
      node.nodeId,
      normalizeTemplateGitUrl(node.pin.url),
    ]),
  );
  ...
  return [...paths]
    .filter((repoPath) => {
      const before = (
        previous?.repositories[repoPath]?.contributions ?? []
      ).map(({ nodeId, subtreeDigest }) => ({
        url: previousUrls.get(nodeId),
        subtreeDigest,
      }));
      const after = (
        inspection.plan.repositories[repoPath]?.contributions ?? []
      ).map(({ nodeId, subtreeDigest }) => ({
        url: nextUrls.get(nodeId),
        subtreeDigest,
      }));
...
export function mergeAcceptedTemplateSuggestion(
  ...
  if (section === "trust") {
    ...
    const merged = Object.fromEntries(
      [...new Set([...Object.keys(current), ...Object.keys(suggestion)])].map(
        (key) => {
          const proposed = suggestion[key];
          if (proposed !== undefined && !Array.isArray(proposed)) {
            throw new Error(
              `Template trust suggestion ${key} must be an array`,
            );
          }
          return [
            key,
  ...
  for (const [key, proposed] of Object.entries(suggestion)) {
    ...
    if (
      prior !== undefined &&
      JSON.stringify(prior) !== JSON.stringify(proposed)
    ) {
      throw new Error(
...
async function inspectAdd(
  ...
  const selectedSources = {
    ...
    resolvePromoted: async (declaration: WorkspaceTemplateDeclaration) =>
      normalizeTemplateGitUrl(declaration.url) ===
      normalizeTemplateGitUrl(selected.url)
        ? selected
...
function protectedMainBuildFailure(
  error: unknown,
): ProtectedMainBuildFailure | null {
    ...
    if (buildFailure && contextId) {
      ...
      return {
        ...
        blocker: {
          ...
          message:
            "The composed workspace needs repair before this template can be installed",
...
async function resumePreparedOperation(
  ...
  if (!affectedRepoPaths) {
    throw new Error(
      `Template operation ${record.operationId} has not finished staging`,
    );
  }
  const contextId = await ensureTemplateOperationContext(
    ctx,
...
export async function cancelTemplateOperation(input: {
  ...
  if (target.applied) {
    throw new Error(
      `Template operation ${input.operationId} is already applied`,
    );
  }
  try {
...
export function assertTemplateOperationCancellable(
  record: TemplateOperationRecord | null,
): void {
...
async function activeTemplateOperations(
  ...
  for (const contextId of listed.contexts) {
    ...
    if (!record) continue;
    if (
      await isTemplateOperationCancelled(ctx, mainEventId, record.operationId)
    )
      continue;
...
export function operationReviewForTemplate(
  ...
  return active.find(({ record }) => {
    ...
    return (
      ...
      normalizeTemplateGitUrl(intent.target.url) ===
        normalizeTemplateGitUrl(node.pin.url)
    );
  });
}
...
export async function activate(ctx: ExtensionContextLike) {
  ...
  const api = {
    async status() {
      ...
      const active = await activeTemplateOperations(
        ctx,
        observation.mainEventId,
      );
      return Promise.all(
          ...
          return {
            ...
            state: missingCredential
              ...
              : reviews.length
                ? ("reviewing" as const)
                : ("current" as const),
            contributedParts: Object.values(
              observation.state?.repositories ?? {},
            ).filter((repository) =>
              repository.contributions.some(
            ...
            suggestions: projected.excludedSuggestions
              ...
              .flatMap((suggestion) =>
                (["trust", "providers"] as const).flatMap((section) =>
                  suggestion[section] === undefined
                    ? []
                    : [{ section, value: suggestion[section] }],
                ),
              ),
          };
        }),
            ...
            ? {
                repair: {
                  ...
                  ...(record.mainAdvanceEventId
                    ? { mainEventId: record.mainAdvanceEventId }
                    : {}),
                  failures:
    ...
    async cancel(input: { operationId: string }) {
      const existing = await readTemplateOperationRecord(
        ctx,
        input.operationId,
      );
      ...
      const result = await cancelTemplateOperation({
        ...
        findContext: async () => {
          ...
          for (const contextId of listed.contexts) {
            const record = await readTemplateOperationRecordInContext(
              ctx,
              contextId,
            );
            ...
            return {
              contextId,
              applied: Boolean(
                record.authoringPublication ??
                (await completedOperationResult(ctx, record)),
              ),
              mainEventId: (
                await ctx.rpc.call<{ mainEventId: string }>(
                  "main",
                  "vcs.status",
                  { contextId },
                )
    ...
    async resume(input: { operationId: string }) {
      ...
      if (!record)
        throw new Error(`Unknown template operation ${input.operationId}`);
      ...
      if (intent.kind === "add") {
        if (!intent.source)
          throw new Error(
            `Template operation ${input.operationId} has no source`,
          );
        return api.add({
      ...
      throw new Error(
        `Template operation ${input.operationId} has unsupported intent`,
      );
    },
    ...
    async check(options: { alias?: string } = {}) {
      ...
      return (env.observation.state?.nodes ?? [])
        ...
        .flatMap((node) => {
          const entry = env.catalog!.entries.find(
            (candidate) =>
              normalizeTemplateGitUrl(candidate.url) ===
              normalizeTemplateGitUrl(node.pin.url),
          );
    ...
    async publishAuthoring(input: {
      ...
      if (existing) {
        ...
        if (!existing.authoringInspection) {
          throw new Error(
            `Template publication command ${input.commandId} has no authoring state`,
          );
        }
        if (existing.authoringPublication) return existing.authoringPublication;
        current = existing.authoringInspection;
      } else {
        current = await inspectTemplateAuthoring(
          ctx,
          observation,
          input.intent,
        );
      ...
      const build = await createAffectedBuildGate(
        ctx,
        observation.localRepoPaths,
      )(contextId, current.includedParts);
      ...
      if (!record) {
        throw new Error(
          `Template publication ${input.commandId} lost its durable operation state`,
        );
      }
      await updateTemplateOperationRecord(ctx, {
        ...record,
    ...
    async suggestRegistryEntry(input: {
      ...
      if (input.catalog.stale || input.catalog.source !== "verified") {
        throw new Error(
          "Refresh the template registry before preparing a contribution",
        );
      }
      ...
      if (!registrySource)
        throw new Error("Workspace does not declare templates.registry");
      if (
        normalizeTemplateGitUrl(registrySource.url) !== current.coordinates.url
      ) {
        throw new Error(
          "The configured template registry does not match the reviewed catalog",
        );
      }
      const publicationUrl = normalizeTemplateGitUrl(
        input.publication.templateUrl,
      );
      ...
      const idCollision = current.entries.find(
        (candidate) =>
          candidate.id === input.entry.id && candidate.url !== publicationUrl,
      );
      if (idCollision) {
        throw new Error(
          `Registry id ${input.entry.id} already belongs to ${idCollision.url}`,
        );
      }
      const urlCollision = current.entries.find(
        (candidate) =>
          candidate.url === publicationUrl && candidate.id !== input.entry.id,
      );
      if (urlCollision) {
        throw new Error(
          `Published template URL already belongs to registry id ${urlCollision.id}`,
        );
      }
      const entry = {
        ...input.entry,
      ...
      const semanticallyChanged =
        canonicalJson(entries) !== canonicalJson(current.entries);
      if (semanticallyChanged && input.revision === current.revision) {
        throw new Error(
          "A changed registry entry requires a new promotion revision",
        );
      }
      if (!semanticallyChanged && input.revision === current.revision) {
        return {
      ...
      const result = await ctx.extensions.invoke<{
      ...
      }>("@workspace-extensions/git-bridge", "suggestRegistryEntry", [
        {
          ...
          registryDocument,
          entryId: entry.id,
          ...(registrySource.credential
            ? { credential: registrySource.credential }
            : {}),
        },
      ]);
      return {
    ...
    async adopt(input: { commandId: string; pin: WorkspaceTemplatePin }) {
      ...
      const recordedPin = record
        ? WorkspaceTemplatePinSchema.parse(
            (record.intent as { target?: unknown }).target,
          )
        : null;
      if (
        recordedPin &&
      ...
      const preview = await inspectAdopt(
        ctx,
        env,
        selectedPin,
        record?.pins ?? [],
    ...
    async pull(input: {
      ...
      if (!node)
        throw new Error(`Unknown installed template alias: ${input.alias}`);
      if (
        requestedPin &&
        normalizeTemplateGitUrl(requestedPin.url) !==
          normalizeTemplateGitUrl(node.pin.url)
      ) {
        throw new Error(
      ...
      const promotedRef =
        recordedPin?.ref ?? requestedPin?.ref ?? entry!.promoted.ref;
    ...
    async remove(input: { commandId: string; alias: string }) {
      ...
      if (
        ...
        throw new Error(
          `Inherited template ${input.alias} cannot be removed directly`,
        );
      }
      const inspection = await inspectTemplateOperation({
        kind: "remove",
    ...
    async decideSuggestion(input: {
      ...
      const node = inspection.plan.nodes.find(
        (candidate) => candidate.alias === input.alias,
      );
      if (!node)
        throw new Error(`Unknown installed template alias: ${input.alias}`);
      ...
      if (value === undefined) {
        throw new Error(
          `Template ${input.alias} has no exact ${input.section} suggestion`,
        );
      }
      const digest = templateSuggestionDigest(
        node.nodeId,
        input.section,
        value,
      );
      ...
      const recorded =
        env.observation.top.templates?.suggestionDecisions?.[decisionKey];
      ...
      if (
        ...
        recorded.decision === normalizedDecision
      ) {
        return {
          operationId: input.commandId,
    ...
    async suggest(input: {
      ...
      const observation = await observeWorkspace(ctx);
      const node = observation.state?.nodes.find(
        (candidate) => candidate.alias === input.alias,
      );
      if (!node)
        throw new Error(`Unknown installed template alias: ${input.alias}`);
