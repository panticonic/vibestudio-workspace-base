...
function calls(result: unknown) {
  ...
  return callMain as typeof callMain & { mock: { calls: [string, unknown[]][] } };
}
...
describe("panel_screenshot", () => {
  it("returns the image alongside a line naming the panel it came from", async () => {
    ...
    expect(callMain.mock.calls[0]).toEqual(["panelCdp.screenshot", ["slot-a", { format: "jpeg" }]]);
    expect(result.content[0]).toMatchObject({ type: "image", data: "aGk=" });
    expect(result.content[1]).toMatchObject({ type: "text", text: expect.stringContaining("800") });
  });

  it("acts on an explicitly named panel when given one", async () => {
    const callMain = calls({ data: "aGk=", mimeType: "image/png", width: 1, height: 1 });
    await createPanelScreenshotTool(callMain, BOUND).execute("t1", { panelId: "slot-other" });
...
describe("panel_console", () => {
  const history = {
    entries: [
      { timestamp: 1, level: "info", message: "booted", line: 1, sourceId: "s", url: "app.js" },
    ],
    errors: [
      { timestamp: 2, level: "error", message: "boom", line: 9, sourceId: "s", url: "app.js" },
    ],
  ...
  it("renders bodies, not counts, and says when history was dropped", async () => {
    ...
    const result = await createPanelConsoleTool(callMain, BOUND).execute("t1", {});
  ...
  it("translates errorsOnly into the host's level filter", async () => {
    const callMain = calls(history);
    await createPanelConsoleTool(callMain, BOUND).execute("t1", { errorsOnly: true, limit: 5 });
    expect(callMain.mock.calls[0]).toEqual([
      "panelCdp.consoleHistory",
      ["slot-a", { limit: 5, levels: ["warning", "error"] }],
    ]);
  });
...
describe("panel_eval", () => {
  it("reports a value with its type", async () => {
    ...
    expect(callMain.mock.calls[0]).toEqual(["panelCdp.evaluate", ["slot-a", "innerWidth", {}]]);
  ...
  it("surfaces a thrown expression as a tool error the model can read", async () => {
    ...
    expect(result.isError).toBe(true);
    expect(result.content[0]).toMatchObject({ text: expect.stringContaining("not a function") });
  });
...
describe("panel_cdp_endpoint", () => {
  it("returns the endpoint and points the model at the cheaper tools", async () => {
    const callMain = calls({ wsEndpoint: "ws://server/cdp/slot-a", token: "t" });
    const tool = createPanelCdpEndpointTool(callMain, BOUND);
    const result = await tool.execute("t1", {});
    expect(callMain.mock.calls[0]).toEqual(["panelCdp.getCdpEndpoint", ["slot-a"]]);
