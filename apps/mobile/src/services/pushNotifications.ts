...
interface NotifeeModule {
  ...
  getDisplayedNotifications?(): Promise<
    Array<{
      id?: string;
      notification?: {
        id?: string;
...
async function getDeviceClientId(): Promise<string> {
  ...
  requireApprovedAppCapability("keychain", "push notification device identity");
  try {
...
export async function registerForPushNotifications(
  ...
  try {
    ...
    callbacks.onToast?.({
      title: "Approval notifications unavailable",
      message:
        "This device could not register for remote approval alerts. It will retry when the connection recovers.",
      tone: "warning",
      durationMs: 8000,
    });
  }
