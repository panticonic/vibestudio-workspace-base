...
import { BROWSER_PRIVACY_FORM_FILL_TYPES } from "@vibestudio/service-schemas/browserPrivacy";
import type {
  MobileBrowserPrivacySection,
  ShellClient,
...
export function BrowserPrivacyManager({
  ...
  const [editingFormFillId, setEditingFormFillId] = useState<number | null>(
    null,
  );
  ...
  const loadMorePasswords = () =>
    void run(async () => {
      const page = await client.listPasswordSummariesPage(
        passwords.length,
        PAGE_SIZE,
      );
  ...
  const loadMoreNeverSave = () =>
    void run(async () => {
      const page = await client.getNeverSaveOriginsPage(
        neverSave.length,
        PAGE_SIZE,
      );
  ...
  const loadMoreFormFill = () =>
    void run(async () => {
      const page = await client.listFormFillValuesPage(
        formFill.length,
        PAGE_SIZE,
      );
  ...
  const loadMoreCookieOrigins = () =>
    void run(async () => {
      const page = await client.listCookieOriginsPage(
        cookieOrigins.length,
        PAGE_SIZE,
      );
  ...
  const saveFormFill = () =>
    void run(
      async () => {
        ...
        if (
          normalizedType &&
          !(BROWSER_PRIVACY_FORM_FILL_TYPES as readonly string[]).includes(
            normalizedType,
          )
        ) {
      ...
      editingFormFillId === null
        ? "Form-fill value saved."
        : "Form-fill value updated.",
    );
  ...
  const inspectSite = () =>
    void run(async () => {
      ...
      if (url.protocol !== "http:" && url.protocol !== "https:") {
        throw new Error(
          "Site inspection supports HTTP and HTTPS origins only.",
        );
      }
        ...
        >
            ...
            <InspectSection
              ...
              onClear={() =>
                ...
                void mutate(
                  ...
                  async () => {
                    const summary = await client.getCookieSiteSummary(
                      inspectResult.origin,
                    );
                    setInspectResult(
                      (current) =>
                        current && {
          ...
          {status ? (
            <Text
              ...
              style={[styles.message, { color: colors.success }]}
            >
              {status}
            </Text>
          ) : null}
          <Text style={[styles.ownershipNote, { color: colors.textTertiary }]}>
            Protected values are available only to this trusted Vibestudio
            shell. Hosted panels and extensions receive no protected rows.
          </Text>
        </ScrollView>
      </View>
    </Modal>
  ...
  return (
    <>
      <Card colors={props.colors}>
        <Text style={[styles.sectionTitle, { color: props.colors.text }]}>
          Saved passwords ({props.passwordTotal})
        </Text>
        {props.passwords.length === 0 ? (
          <Empty colors={props.colors} text="No saved passwords." />
        ) : (
          props.passwords.map((password) => (
            ...
            >
              <View style={styles.rowCopy}>
                ...
                <Text
                  style={[styles.body, { color: props.colors.textSecondary }]}
                >
              ...
              <Button
                ...
                variant="danger"
                disabled={props.busy}
                onPress={() => props.onDeletePassword(password)}
              />
            </View>
          ))
        )}
        {props.passwords.length < props.passwordTotal ? (
          <Button
            label="Load more passwords"
            disabled={props.busy}
            onPress={props.onLoadMorePasswords}
          />
        ) : null}
      </Card>
      <Card colors={props.colors}>
        <Text style={[styles.sectionTitle, { color: props.colors.text }]}>
          Never save ({props.neverSaveTotal})
        </Text>
        {props.neverSave.length === 0 ? (
          <Empty colors={props.colors} text="No never-save rules." />
        ) : (
          props.neverSave.map((origin) => (
            <View
              key={origin}
              style={[
                styles.row,
            ...
            >
              ...
              <Button
                label="Remove"
                disabled={props.busy}
                onPress={() => props.onRemoveNeverSave(origin)}
              />
            </View>
          ))
        )}
        {props.neverSave.length < props.neverSaveTotal ? (
          <Button
            label="Load more never-save rules"
            disabled={props.busy}
            onPress={props.onLoadMoreNeverSave}
          />
        ) : null}
      </Card>
    </>
    ...
    <>
      <Card colors={props.colors}>
        <Text style={[styles.sectionTitle, { color: props.colors.text }]}>
          {props.editingId === null
            ? "Add form-fill value"
            : "Edit form-fill value"}
        </Text>
        <LabeledInput
          colors={props.colors}
          label="Field name"
          value={props.fieldName}
          onChangeText={props.onFieldName}
          editable={props.editingId === null}
          placeholder="email"
        />
        <LabeledInput
          colors={props.colors}
          ...
          onChangeText={props.onFieldType}
          editable={props.editingId === null}
          placeholder="email"
        />
        <LabeledInput
          ...
          onChangeText={props.onFieldValue}
        ...
        <LabeledInput
          ...
          onChangeText={props.onFieldLabel}
          placeholder="Work email"
        />
        <View style={styles.actions}>
          <Button
            label={props.editingId === null ? "Add value" : "Save changes"}
            variant="filled"
            disabled={props.busy}
      ...
      <Card colors={props.colors}>
        ...
        {props.values.length === 0 ? (
          <Empty colors={props.colors} text="No saved form-fill values." />
        ) : (
          props.values.map((value) => (
            ...
            >
              <Text style={[styles.rowTitle, { color: props.colors.text }]}>
                {value.displayLabel || value.fieldName}
              </Text>
              <Text
                selectable
                style={[styles.protectedValue, { color: props.colors.text }]}
              >
                {value.value}
              </Text>
              <Text
              ...
              <View style={styles.actions}>
                <Button
                  label="Edit"
                  disabled={props.busy}
                  onPress={() => props.onEdit(value)}
                />
                <Button
                  label="Delete"
                  variant="danger"
                  disabled={props.busy}
                  onPress={() => props.onDelete(value)}
                />
              </View>
            </View>
          ))
        )}
        {props.values.length < props.total ? (
          <Button
            label="Load more form-fill values"
            disabled={props.busy}
    ...
    <Card colors={props.colors}>
      ...
      {props.result ? (
        <View
          accessibilityLiveRegion="polite"
          style={[
            styles.inspectResult,
            { backgroundColor: props.colors.surfaceSunken },
          ]}
        >
          <Text style={[styles.rowTitle, { color: props.colors.text }]}>
            {props.result.origin}
          </Text>
          <Text style={[styles.body, { color: props.colors.textSecondary }]}>
            {props.result.passwordCount} saved password
            {props.result.passwordCount === 1 ? "" : "s"}
          </Text>
          <Text style={[styles.body, { color: props.colors.textSecondary }]}>
            {props.result.cookieCount} cookie
            {props.result.cookieCount === 1 ? "" : "s"}
          </Text>
          <Button
            label="Clear site cookies"
            variant="danger"
            disabled={props.busy || props.result.cookieCount === 0}
            onPress={props.onClear}
          />
        </View>
      ) : null}
  ...
  return (
    <>
      <Card colors={props.colors}>
        ...
        <Text style={[styles.body, { color: props.colors.textSecondary }]}>
          The canonical vault currently has cookies for {props.total} origin
          {props.total === 1 ? "" : "s"} (revision {props.revision}). Mobile
          WebViews keep their own isolated site data; these controls manage the
          shared vault and desktop browser projection.
        </Text>
        <View style={styles.actions}>
          <Button
            label="End session"
      ...
      <Card colors={props.colors}>
        <Text style={[styles.sectionTitle, { color: props.colors.text }]}>
          Cookie origins ({props.total})
        </Text>
        {props.origins.length === 0 ? (
          <Empty colors={props.colors} text="No shared browser cookies." />
        ) : (
        ...
        {props.origins.length < props.total ? (
          <Button
            label="Load more cookie origins"
            disabled={props.busy}
            onPress={props.onLoadMore}
          />
        ) : null}
...
function Empty({ colors, text }: { colors: ThemeColors; text: string }) {
  return (
    <Text style={[styles.empty, { color: colors.textSecondary }]}>{text}</Text>
  );
}
