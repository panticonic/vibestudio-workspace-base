 ...
 */
import { type RpcClient, type RpcEnvelope } from "@vibestudio/rpc";
...
type IncomingPanelLocationBridge = {
  ...
  openLocation: (location: PanelLocation) => Promise<void>;
};
...
/**
 * One immutable workspace client owns its caches, event subscriptions and native
 * presentation declarations. Captured handlers keep this instance when another
 * workspace receives focus. The supplied RPC session must address this workspace.
 */
export function createShellWorkspaceClient(
  rpc: RpcClient,
  ownership: {
  ...
  const app = {
    ...
    openShellSurface: (target: ShellSurfaceTarget) =>
      appClient.openShellSurface(target),
  ...
  async function localPresentationKey(
    ...
    const workspaceId = await ownership.workspaceId;
  ...
  const panel = {
    ...
    getTreePage: (input: Parameters<typeof workspacePresentation.page>[0]) =>
      workspacePresentation.page(input),
    getTreePath: (panelId: string) => workspacePresentation.path(panelId),
    searchTree: (
    ...
    setFocusedPanelId: (panelId: string) =>
      viewClient.setFocusedPanelId(panelId),
    focus: focusPanel,
  ...
  const view = {
    ...
    hideContentOverlay: (surface: string) =>
      viewClient.hideContentOverlay({ surface }),
   ...
   */
  const incomingShellSurface = {
    getPending: () =>
      g.__vibestudioIncomingShellSurface?.getPending() ?? Promise.resolve(null),
  };
  const incomingPanelLocation = {
    getPending: () =>
      g.__vibestudioIncomingPanelLocation?.getPending() ??
      Promise.resolve(null),
    onLocation: (handler: (location: PanelLocation) => void) =>
      g.__vibestudioIncomingPanelLocation?.onLocation(handler) ?? (() => {}),
    openLocation: (location: PanelLocation) =>
      g.__vibestudioIncomingPanelLocation?.openLocation(location) ??
      Promise.reject(new Error("Panel navigation host is unavailable")),
  };
  const menu = {
  ...
  const workspace = {
    ...
    select: async (name: string) => {
      ...
      return hubControlClient.routeWorkspace({
  ...
  const hubControl = hubControlClient;
  // =============================================================================
  // Autofill Service
  // =============================================================================
  const autofill = {
  ...
  const ACCOUNT_PROFILE_CHANGED_EVENT = "account-profile-changed";
  ...
  const browserData = browserDataClient;
  const browserEnvironment = browserEnvironmentClient;
  // =============================================================================
  // Runtime supervision
  // =============================================================================
  const supervisedUnits = {
