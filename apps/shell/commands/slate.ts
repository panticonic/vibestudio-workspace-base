 ...
 */
function desktopRuns(client: ShellWorkspaceClient): Record<string, SlateRun> {
  const { app, panel, quickfire, workspace } = client;
  return {
    // ---- Panel ---------------------------------------------------------------
    ...
    "panel.duplicate": async () => {
      ...
      await panel.createPanel(observation.source, {
        isRoot: false,
        focus: true,
      });
      return { close: true };
    ...
    "quickfire.promote": async () => {
      ...
      if (!promoted)
        return { message: "This panel has no conversation to open" };
...
export function buildSlate(deps: SlateDeps): SlateCommand[] {
  const runs = desktopRuns(deps.client);
  return buildSlateDefinitions({ workspaceNames: deps.workspaceNames })
    ...
    .map((spec) => {
      ...
      if (!run)
        throw new Error(`Desktop slate has no implementation for "${spec.id}"`);
      return { ...spec, run };
    });
}
...
export async function runContributedCommand(
  ...
  await client.hostCommands.run(panelId, commandId);
  return { close: true };
}
